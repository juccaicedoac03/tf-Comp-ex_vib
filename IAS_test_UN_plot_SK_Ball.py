...
# %% loading data
filed = 'Data/Test_18_BALL_STASK'
mat_contents = sio.loadmat(filed, mdict=None, appendmat=True)

fs = int(mat_contents['fs_ori'])
data = mat_contents['data_save'][:, 4]
f_est = mat_contents['f_est_save'][:, 4]
...
plt.vlines(x=xcoords,ymin=0, ymax=1e-4, color='red', linestyle=':',label= 'BSF')

Bfq = .4
plt.axis([freq[0], 5, 0, 1.1 * max(sp[(freq >0) * (freq < 100)])])
xcoords = np.r_[:30 * Bfq:Bfq] + Bfq
plt.vlines(x=xcoords,ymin=0, ymax=1e-4, color='green',label= 'FTF',linestyle='dashdot')
plt.legend(loc='upper right', shadow=True)
#plt.savefig('figures/Test_UN_BALL_baseline00.pdf', bbox_inches='tight')
...
#plt.savefig('figures/Test_UN_BALL_baseline01.pdf', bbox_inches='tight')
# %% Proposed STASK
filed = 'Data/Test_18_BALL_STASK'
mat_contents = sio.loadmat(filed, mdict=None, appendmat=True)
...
sp = mat_contents['psd_env_save'][:,4]
...
#plt.savefig('figures/Test_UN_BALL_STASK.pdf', bbox_inches='tight')